### Adapted from https://github.com/karlicoss/orger/blob/master/src/orger/inorganic.py
...
def asorgoutline(
        ...
        timestamps: Optional[Sequence[Dateish]] = None,
    ...
    time_lines = [] if timestamps is None else [        
        timestamp(time[0], active=True) + ('--'+timestamp(time[1], active=True) if time[1] else '') for time in timestamps
    ]
    ...
    if body is not None and not escaped:
        body = _sanitize_body(body, time_lines)
...
class OrgNode():
    ...
    scheduled: Optional[Dateish] = None
    timestamps: Optional[Sequence[Tuple[Dateish, Dateish]]] = None
    properties: Optional[Mapping[str, str]] = None
    # TODO make body lazy as well?
    body: Optional[str] = None
    children: Sequence['OrgNode'] = field(default_factory=list)

    # NOTE: this is a 'private' attribute
    escaped: bool=False

    @classmethod
    def from_orgparse(class_object, node, **kwargs):
        """
        Construct an OrgNode from an argparse node, modification can be applied through kwargs 
        """
...
def _sanitize_body(text: str, timestamps: []) -> str:
    r"""
